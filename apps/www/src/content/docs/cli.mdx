...
The `nimbus-docs` CLI does four things: list what's in the registry, install it, lint your MDX content, and clean the build cache.

## `nimbus-docs list`

Lists every installable component, utility, and feature.

<PackageManagers pkg="nimbus-docs" type="dlx" args="list" />
...
<PackageManagers pkg="nimbus-docs" type="dlx" args="add badge" />
...
Removes the incremental-build cache — run it if you suspect cache corruption or after a framework upgrade. It's safe any time; the cache rebuilds on the next `astro build`.

```bash
nimbus-docs clean
```

This clears `node_modules/.astro/nimbus` and the `.nimbus/cache` fallback. If you've set a custom Astro `cacheDir`, remove `<cacheDir>/nimbus` manually — the standalone CLI doesn't load your Astro config.
