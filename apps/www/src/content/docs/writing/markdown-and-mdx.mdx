...
:::warning[Say "hello"]
Quotes are preserved in titles. The `warning` alias uses the caution variant.
:::

:::danger[`Cache-Control` header]
Inline code renders in the title, and body formatting such as **this** still works.
:::
...
Titles support inline code, emphasis, strong text, strikethrough, and links,
including reference links. Static phrasing HTML such as `<code>` is also supported.
Nimbus parses and sanitizes this formatting, then supplies a `title-content` slot
and a plain-text `title` fallback to your Aside component. Expressions are not
evaluated, and unsafe HTML is removed.

Existing projects need the small [Aside slot update](/components/aside#formatted-title)
to display formatted titles. Without it, titles remain readable plain text.
Direct `<Aside title="...">` calls keep their plain-string behavior.

Directive titles must still be valid MDX syntax because Sätteri parses the document
before the AST pass runs. For arbitrary text such as an unclosed `<T>` or `{`, use
the direct component's string prop:
