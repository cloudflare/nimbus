...
<PackageManagers pkg="nimbus-docs" type="dlx" args="add 404-page" />
...
The moment agents start drafting pages, the question stops being *can a machine read this* and becomes *who wrote this, and has a human checked it.* Nimbus answers that in the schema, not by convention.

The default is inverted: agent-facing is assumed, so `audience: "human"` is the deliberate flag for a page written first for people. `aiGenerated` marks a page an agent drafted but no human has reviewed yet — the layout surfaces an "awaiting review" label until someone signs off. The origin and review state of every page travel with the page itself.
